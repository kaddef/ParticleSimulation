...
ThreadedSolver::ThreadedSolver(ThreadPool &threadPool):
...
void ThreadedSolver::InitiateParticles(int maxCount, bool isDrawingImage) {
...
void ThreadedSolver::ParticleUpdate(int id, float dt) {
    Particle &particle = this->particles[id];
    particle.Update(dt);
    UpdateParticleInGrid(&particle, particle.GetPosition());
}

void ThreadedSolver::ThreadedUpdate(float dt) {
    threadPool.Parallel(static_cast<int>(particles.size()), [&](int start, int end) {
        for (int i = start; i < end; i++) ParticleUpdate(i, dt);
    });
}

void ThreadedSolver::UpdateObjects(float dt) { // OLD
    for (Particle &particle : this->particles) {
        particle.Update(dt);
        UpdateParticleInGrid(&particle, particle.GetPosition());
    }
...
void ThreadedSolver::Update() {
    ...
    for (int i = 0; i < substeps; i++) {
        ...
        this->ApplyGravity();
        this->ThreadedBoundaryCheck();
        // this->SolveCollision();
        this->SpatialCollision();
        this->ThreadedUpdate(substepDt);// this->UpdateObjects(substepDt);
    }

}

void ThreadedSolver::ParticleBoundaryCheck(int id) {
...
void ThreadedSolver::ThreadedBoundaryCheck() {
    threadPool.Parallel(static_cast<int>(particles.size()), [&](int start, int end) {
            for (int i = start; i < end; i++) ParticleBoundaryCheck(i);
    });
}

void ThreadedSolver::SolveCollision() {
...
void ThreadedSolver::DrawParticles() {
    for (Particle &particle : this->particles) {
        particle.Render();
    }
}

void ThreadedSolver::DrawBoundary() {
    DrawCircleV(boundary.position, boundary.radius, BLACK);
}

void ThreadedSolver::SetBoundary(const Vector2 position, const float radius) {
    this->boundary.position = position;
    this->boundary.radius = radius;
}

void ThreadedSolver::SetObjectVelocity(Particle& object, Vector2 vel) {
    object.SetVelocity(vel, dt / substeps);
}

int ThreadedSolver::GetParticleCount() const {
...
void ThreadedSolver::SetGravity(Vector2 gravity) {
    this->gravity = gravity;
}

void ThreadedSolver::GetCurrentState() {
...
void ThreadedSolver::LoadColorsFromFile() {
...
Key ThreadedSolver::GetKey(float x, float y) {
    return Key(
...
Key ThreadedSolver::CreateKey(int gridX, int gridY) {
    return Key(gridX, gridY);
}

void ThreadedSolver::AddParticleToGrid(Particle *p) {
...
void ThreadedSolver::UpdateParticleInGrid(Particle *p, Vector2 newPos) {
    std::lock_guard<std::mutex> lock(gridAccesMutex);
...
void ThreadedSolver::CollideCells(Key key1, Key key2) {
...
std::vector<Key> ThreadedSolver::GetNeighboringCells(Key& key) {
    int cellCountInRow = GetScreenWidth() / cellSize;
